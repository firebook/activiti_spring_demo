          ...
          <table border="0" cellpadding="0" cellspacing="0"  class="tableBD">
            ...
            <%for(ProcessInstance d : list){%>
				<tr>
					...
					<td ></td>
					<td >
						<a href="<%=request.getContextPath() %>/jsp/processInstance.jsp?processInstanceId=<%=d.getId() %>&doWhat=delete"/>删除</a>
					</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
					...
					<td>
					<%if(execution != null){%>
						<a href="<%=request.getContextPath() %>/jsp/processInstance.jsp?doWhat=signal&executionId=<%=execution.getId() %>"/>解除等待</a>
					<%} %>
					</td>
				</tr>
			<%} %>
